...
{
    public partial class SeasonMenuBrioche : Form
    {
        ...
        {
            var establishment = SeasonMenuB.GetBriocheEstablishment();
            if (establishment == null)
            {
                MessageBox.Show("Ошибка: заведение не загружено", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            allDishes = establishment.RegularMenu?.Dishes ?? new List<Dish>();
            allDishes = allDishes.Concat(establishment.SeasonMenu?.Dishes ?? new List<Dish>()).ToList();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            using (var addForm = new AddDishGastronomySM())
            {
                if (addForm.ShowDialog() == DialogResult.OK && addForm.dish != null)
                {
                    ...
                    SeasonMenuB.Save(establishment);
        ...
        private void SeasonMenuBrioche_Load(object sender, EventArgs e)
        {
